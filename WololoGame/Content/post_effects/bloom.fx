...
float4 PixelShaderBlurH(VertexShaderOutput input) : COLOR0
{
    ...
    color += (tex2D(originalTexSampler, texel.texCoord1) - threshold) / (1 - threshold) * weight4;
    color += (tex2D(originalTexSampler, texel.texCoord2) - threshold) / (1 - threshold) * weight3;
    color += (tex2D(originalTexSampler, texel.texCoord3) - threshold) / (1 - threshold) * weight2;
    color += (tex2D(originalTexSampler, texel.texCoord4) - threshold) / (1 - threshold) * weight1;
    color += (tex2D(originalTexSampler, texel.texCoord5) - threshold) / (1 - threshold) * weight0;
    color += (tex2D(originalTexSampler, texel.texCoord6) - threshold) / (1 - threshold) * weight1;
    color += (tex2D(originalTexSampler, texel.texCoord7) - threshold) / (1 - threshold) * weight2;
    color += (tex2D(originalTexSampler, texel.texCoord8) - threshold) / (1 - threshold) * weight3;
    color += (tex2D(originalTexSampler, texel.texCoord9) - threshold) / (1 - threshold) * weight4;
...
float4 adjustSat(float4 color, float sat)
{
	float grey = dot(color, float3(0.3,0.59,0.11));
	return lerp(grey, color, sat);
}

float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
	float4 image = tex2D( originalTexSampler, input.TexCoord.xy);
	float4 blurred = tex2D( blurredTexSampler, input.TexCoord.xy);

	float4 original = adjustSat(image, originalSaturation) * originalIntensity;
	float4 bloom = adjustSat(blurred, bloomSaturation) * bloomIntensity;

	original *= (1 - saturate(bloom));
